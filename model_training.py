...
pipeline = Pipeline(stages=[hashingTF, idf, indexer])

# Fit the pipeline and transform the data
model = pipeline.fit(df)
result = model.transform(df)

# Convert the array of filtered words to a string (optional)
...
# Show the resulting features and labels
result.select("id", "filtered_words_str", "features_str", "label_index").show(5)

# Save the resulting features and labels to task3_output.csv (overwrite if it exists)
result.select("id", "filtered_words_str", "features_str", "label_index") \
    .write.option("header", "true") \
    .mode("overwrite") \
    .csv("output/task3_output.csv")

# Now, prepare the data for model training
...
# Write predictions to task4_output.csv
